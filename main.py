...
################################################################################
def AsymmetricLoss(gamma_neg=2.0, gamma_pos=1.0):
...
def load_data(path_to_data):
    ...
    X_train_annotation = np.transpose(np.array(data['train_in_annotation']),
                                      axes=(0, 2, 1))
    X_train = np.concatenate((X_train_seq, X_train_region[:,50:200,:] ,X_train_annotation), axis=2)
    ...
    X_valid_region = np.transpose(np.array(data['valid_in_region']),
                                  axes=(0, 2, 1))
    X_valid_annotation = np.transpose(np.array(data['valid_in_annotation']),
                                      axes=(0, 2, 1))
    X_valid = np.concatenate((X_valid_seq, X_valid_region[:,50:200,:] ,X_valid_annotation), axis=2)
    ...
    X_test_region = np.transpose(np.array(data['test_in_region']),
                                  axes=(0, 2, 1))
    X_test_annotation = np.transpose(np.array(data['test_in_annotation']),
                                      axes=(0, 2, 1))
    X_test = np.concatenate((X_test_seq, X_test_region[:, 50:200, :], X_test_annotation), axis=2)
...
# Choose what attention_module to use: cbam_block / se_block / eca
attention_module = None
model_type = base_model if attention_module == None else base_model+'_'+attention_module

# Load the data.
print ('loading data')
path_data = "/Users/storm/reproduce/textCNN/Data/data_RBPslow.h5"
...
print ("generate the class-aware weights")
...
model = residualbind.ResidualBind(input_shape=input_shape, num_class= num_task, classification=True, attention_module = attention_module)
