...
#
################################################################################
def load_data_seq_region(path_to_data):
...
### Model only using sequence and region data
def create_model_seq_region(num_task, input_len_l, input_len_r):
		K.clear_session()
...
def train_model_seq_region(data_path, model_funname,res_path, model_name,num_task,input_len_l,input_len_r, num_epoch, batchsize, model_path="./weights.hdf5", plot=False):

	print ('creating model')
	if isinstance(model_funname, str):
		dispatcher={'create_model1':create_model_seq_region}
		try:
	...
	print ('loading data')
	X_train_seq,  X_train_region, y_train, X_valid_seq, X_valid_region, y_valid, X_test_seq, X_test_region, y_test=load_data_seq_region(data_path)

	print("data shape:",X_train_seq.shape, X_train_region.shape)
...
def main():
	...
	train_model_seq_region(data_path=args.data_path, model_funname=funname, res_path=args.output_path, model_name=args.model_name,num_task=args.num_task, input_len_l=args.input_len_l, input_len_r=args.input_len_r,
