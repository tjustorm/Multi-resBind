...
#
################################################################################
def load_data_struc_region(path_to_data):
    ...
    data.close()
    return X_train_struc, X_train_region, y_train, X_valid_struc, X_valid_region, y_valid, X_test_struc, X_test_region, y_test
...
### Model only using structure and region data
def create_model_struc_region(num_task, input_len_l, input_len_r):
		K.clear_session()
...
def train_model_struc_region(data_path, model_funname,res_path, model_name,num_task,input_len_l,input_len_r, num_epoch, batchsize, model_path="./weights.hdf5", plot=False):

	print ('creating model')
	if isinstance(model_funname, str):
		dispatcher={'create_model1':create_model_struc_region}
		try:
	...
	print ('loading data')
	X_train_struc,  X_train_region, y_train, X_valid_struc, X_valid_region, y_valid, X_test_struc, X_test_region, y_test=load_data_struc_region(data_path)

	print("data shape:",X_train_struc.shape, X_train_region.shape)
	...
	history = model.fit([X_train_struc, X_train_region], y_train, epochs=num_epoch, batch_size=batchsize,validation_data=([X_valid_struc, X_valid_region],y_valid), class_weight=class_weight, verbose=2, callbacks=[checkpointer,earlystopper, tb])

	print ('saving the model')
	model.save(os.path.join(res_path, model_name + ".h5"))

	print ('testing the model')
	score = model.evaluate([X_test_struc, X_test_region], y_test)
...
def test_model(output_path,data_path, model_path, model_funname,res_path,model_name,input_len_l,input_len_r, num_task):
	...
	y_pred = model.predict([X_test_struc, X_test_region], verbose=1)
	model.evaluate([X_test_struc, X_test_region], y_test)
...
def main():
	...
	train_model_struc_region(data_path=args.data_path, model_funname=funname, res_path=args.output_path, model_name=args.model_name,num_task=args.num_task, input_len_l=args.input_len_l, input_len_r=args.input_len_r,
