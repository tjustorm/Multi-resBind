...
def load_data_region(path_to_data):
    ...
    return X_train_region, y_train, X_valid_region, y_valid, X_test_region, y_test
...
def train_diff_model(data_path, model_funname, res_path, model_name, num_task,
    ...
    print ('loading data')
    X_train_region, y_train, X_valid_region, y_valid, X_test_region, y_test = load_data_region(data_path)

    print("input shape:", X_train_region.shape)
    ...
    history = model.fit(X_train_region, y_train,
                        epochs=num_epoch, batch_size=batchsize,
                        validation_data=(X_valid_region, y_valid),
    ...
    score = model.evaluate(X_test_region, y_test)
...
def test_model(output_path, data_path, model_path, model_funname, res_path,
    ...
    y_pred = model.predict(X_test_region, verbose=1)
    model.evaluate(X_test_region, y_test)
